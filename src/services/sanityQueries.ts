...
const fetchFromSanity = async <T>(query: string, params?: Record<string, unknown>): Promise<T> => {
  ...
  try {
    // Cast to any to bypass strict overloads, but keep the runtime call intact
    const result = await (sanityClient.fetch as any)(query, params);
    // eslint-disable-next-line no-console
    console.log('Sanity fetch success, result count:', Array.isArray(result) ? result.length : 'non-array');
    return result;
  } catch (err) {
    // eslint-disable-next-line no-console
